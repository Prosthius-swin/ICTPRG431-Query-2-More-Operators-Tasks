...
CREATE DATABASE Query_2_More_Operators_Tasks;
GO
USE Query_2_More_Operators_Tasks;
GO
...
SELECT M.*
FROM MOVIE9468 M
WHERE UPPER(RATINGCODE) = 'MA' AND RUNTIME <= 100
ORDER BY MOVIENO ASC

-- 4
SELECT M.*
FROM MOVIE9468 M
WHERE (UPPER(RATINGCODE) = 'PG' AND TMDB_SCORE > 7.6) 
OR 
(UPPER(RATINGCODE) = 'M' AND TMDB_SCORE > 7.8)
ORDER BY MOVIENO ASC
...
SELECT M.*
FROM MOVIE9468 M
WHERE RELYEAR IN (2014, 2015) AND RUNTIME > 130
ORDER BY MOVIENO ASC
